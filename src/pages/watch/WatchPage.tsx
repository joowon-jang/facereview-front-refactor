...
import { CommentType, EmotionType, VideoDetailType, VideoRelatedType } from 'types';
import { getDistributionToGraphData, getTimeToString, mapNumberToEmotion } from 'utils/index';
...
const WatchPage = (): ReactElement => {
  ...
  const { is_sign_in, access_token, user_profile, user_announced, setUserAnnounced } = useAuthStorage(
    ({ is_sign_in, access_token, user_profile, user_announced, setUserAnnounced }) => ({
      is_sign_in,
      access_token,
      user_profile,
      user_announced,
  ...
  const [currentMyEmotion, setCurrentMyEmotion] = useState<EmotionType>('neutral');
  const [currentOthersEmotion, setCurrentOthersEmotion] = useState<EmotionType>('neutral');
  ...
  }, [access_token, capture, myGraphData, othersGraphData, video, videoData?.youtube_index]);
  ...
  const GraphDetailDataItem = ({
    ...
    return (
      <div className={`graph-detail-item ${emotion === mostEmotion ? 'active' : null}`}>
        <EmotionBadge type={'big'} emotion={emotion} />

        <p className="font-label-medium emotion-text">{emotionText}</p>
        <p className="font-label-medium emotion-percentage">{'' + graphData[0][emotion] + '%'}</p>
      </div>
    );
  };
  ...
  const CommentItem = ({
    ...
    return (
      <div
        ...
        <ProfileIcon type={'icon-small'} color={mapNumberToEmotion(user_profile)} style={{ marginRight: '12px' }} />
        <div className="comment-right-container">
          <div className="comment-text-wrapper">
            ...
            <div className="comment-text font-body-medium">{comment_contents}</div>
          </div>

          <div className="comment-icon-container">
            {hoveredComment === comment_index && (
              <SomeIcon type={'more'} onClick={() => setIsEditVisible(comment_index)} />
            )}
            <div className={`comment-edit-container ${isEditVisible === comment_index && 'visible'}`}>
              <div
  ...
  useEffect(() => {
    if (editingcommentindex !== null) {
      commentList.map(
        (item, i) => item.comment_index === editingcommentindex && setModifyingComment(commentList[i].comment_contents)
      );
    }
  ...
  const renderMobileContainer = () => {
    return (
      <div className="watch-page-cam-container">
        ...
        <div className="emotion-container">
          <div className="emotion-title-wrapper">
            <h4 className="emotion-title font-title-small">실시간 다른 사람들의 감정</h4>
  ...
  return (
    <div className="watch-page-container">
      <ModalDialog type={'one-button'} name="watch-page-modal" isOpen={isModalOpen1} onClose={closeModal1}>
        <div className="watch-page-modal-image-container">
          <img className="watch-page-modal-image" src={safeImage} alt="safe-img" />
        </div>
        <div className="watch-page-modal-label-container">
          <h2 className="watch-page-modal-title font-title-medium">안심하세요!</h2>
          <p className="font-body-large">영상 시청 중의 나의 모습은 기록되거나 저장되지 않아요.</p>
        </div>
      </ModalDialog>
      <div className="main-container">
        <div className="video-fixed-container">
          ...
          <div className="video-information-container">
            <div className={isMobile ? 'title font-title-small' : 'title font-title-medium'}>
            ...
            <div className="right-side">
              ...
              />
              <p className="video-hits-text font-label-small">조회수 {videoData?.youtube_hits || 0}회</p>
            </div>
          </div>
          {isMobile && <Devider />}
        </div>
        ...
        {isMobile && <Devider style={{ width: '100vw', marginLeft: '-16px' }} />}
        ...
        <div className="comment-container">
          ...
          <div className={isMobile ? 'comment-info-text font-title-mini' : 'comment-info-text font-title-small'}>
                  ...
                  <div className="comment-modifying-container">
                    ...
                    <div className="comment-modifying-wrapper">
                      <div className="comment-modifying-info-wrapper">
                        <div className="comment-modifying-nickname font-label-small">{comment.user_name}</div>
                      ...
                      <div className="comment-modifying-button-wrapper">
                        ...
                        <div
                          ...
                          style={{
                            pointerEvents: modifyingComment.length > 0 ? 'auto' : 'none',
                          }}>
                          저장
                        </div>
                      </div>
      ...
      <div className="side-container">
        ...
        <div className="recommend-container">
          <h4 className="recommend-title font-title-small">이 영상은 어때요?</h4>
